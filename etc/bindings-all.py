...
def run(cmd, **kwargs):
    ...
    subprocess to complete, and throw and exception if it didn't exit with 0.
...
for (build_modifier, extra_cargo_flags) in BUILDS:
    ...
    try:
        ...
        # date, but we need to build the mozjs_sys dependency so we can
        # regenerate the bindings for it.
        pass
