...
impl TypedArrayElement for ArrayBufferViewU8 {
    ...
    unsafe fn length_and_data(obj: *mut JSObject) -> (*mut Self::Element, u32) {
        ...
        let mut shared = false;
        let mut data = ptr::null_mut();
        GetArrayBufferViewLengthAndData(obj, &mut len, &mut shared, &mut data);
        assert!(!shared);
        (data, len)
    }
}
...
macro_rules! typedarray {
    (in($cx:expr) let $name:ident = $init:expr) => {
        ...
        let $name = $crate::typedarray::TypedArray::new($cx, &mut __root);
    };
    (in($cx:expr) let mut $name:ident = $init:expr) => {
        let mut __root = $crate::jsapi::Rooted::new_unrooted($init);
        let mut $name = $crate::typedarray::TypedArray::new($cx, &mut __root);
    }
}
